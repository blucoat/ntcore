...
class SavePersistentImpl {
 public:
  ...
  explicit SavePersistentImpl(wpi_llvm::raw_ostream& os) : m_os(os) {}

  void Save(wpi_llvm::ArrayRef<Entry> entries);

 private:
  void WriteString(wpi_llvm::StringRef str);
  void WriteHeader();
  void WriteEntries(wpi_llvm::ArrayRef<Entry> entries);
  void WriteEntry(wpi_llvm::StringRef name, const Value& value);
  bool WriteType(NT_Type type);
  void WriteValue(const Value& value);

  wpi_llvm::raw_ostream& m_os;
};

}  // namespace

/* Escapes and writes a string, including start and end double quotes */
void SavePersistentImpl::WriteString(wpi_llvm::StringRef str) {
  m_os << '"';
  for (auto c : str) {
    switch (c) {
      case '\\':
      ...
      default:
        ...
        m_os << "\\x";
        m_os << wpi_llvm::hexdigit((c >> 4) & 0xF);
        m_os << wpi_llvm::hexdigit((c >> 0) & 0xF);
    }
  }
  m_os << '"';
}

void SavePersistentImpl::Save(wpi_llvm::ArrayRef<Entry> entries) {
  WriteHeader();
...
void SavePersistentImpl::WriteEntries(wpi_llvm::ArrayRef<Entry> entries) {
  for (auto& i : entries) {
...
void SavePersistentImpl::WriteEntry(wpi_llvm::StringRef name, const Value& value) {
...
void SavePersistentImpl::WriteValue(const Value& value) {
  switch (value.type()) {
    case NT_BOOLEAN:
      ...
      break;
    case NT_DOUBLE:
      m_os << wpi_llvm::format("%g", value.GetDouble());
      break;
    case NT_STRING:
...
void Storage::SavePersistent(wpi_llvm::raw_ostream& os, bool periodic) const {
...
const char* Storage::SavePersistent(const Twine& filename,
                                    bool periodic) const {
  wpi_llvm::SmallString<128> fn;
  filename.toVector(fn);
  wpi_llvm::SmallString<128> tmp = fn;
  tmp += ".tmp";
  wpi_llvm::SmallString<128> bak = fn;
  bak += ".bak";
  ...
  std::error_code ec;
  wpi_llvm::raw_fd_ostream os(tmp, ec, wpi_llvm::sys::fs::F_Text);
  if (ec.value() != 0) {
...
void Storage::SaveEntries(wpi_llvm::raw_ostream& os, const Twine& prefix) const {
  std::vector<SavePersistentImpl::Entry> entries;
