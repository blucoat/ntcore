...
std::shared_ptr<Value> Value::MakeBooleanArray(wpi_llvm::ArrayRef<int> value,
                                               uint64_t time) {
...
std::shared_ptr<Value> Value::MakeDoubleArray(wpi_llvm::ArrayRef<double> value,
                                              uint64_t time) {
...
std::shared_ptr<Value> Value::MakeStringArray(wpi_llvm::ArrayRef<std::string> value,
                                              uint64_t time) {
...
void nt::ConvertToC(wpi_llvm::StringRef in, NT_String* out) {
  out->len = in.size();
...
std::shared_ptr<Value> nt::ConvertFromC(const NT_Value& value) {
  switch (value.type) {
    ...
    case NT_BOOLEAN_ARRAY:
      return Value::MakeBooleanArray(wpi_llvm::ArrayRef<int>(
          value.data.arr_boolean.arr, value.data.arr_boolean.size));
    case NT_DOUBLE_ARRAY:
      return Value::MakeDoubleArray(wpi_llvm::ArrayRef<double>(
