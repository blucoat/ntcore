...
struct NT_String* NT_GetStringForTesting(const char* string, int* struct_size) {
  ...
  nt::ConvertToC(wpi_llvm::StringRef(string), str);
...
struct NT_EntryInfo* NT_GetEntryInfoForTesting(const char* name,
  ...
  nt::ConvertToC(wpi_llvm::StringRef(name), &entry_info->name);
...
struct NT_ConnectionInfo* NT_GetConnectionInfoForTesting(
  ...
  nt::ConvertToC(wpi_llvm::StringRef(remote_id), &conn_info->remote_id);
  nt::ConvertToC(wpi_llvm::StringRef(remote_ip), &conn_info->remote_ip);
...
struct NT_Value* NT_GetValueStringForTesting(uint64_t last_change,
  ...
  value->last_change = last_change;
  nt::ConvertToC(wpi_llvm::StringRef(str), &value->data.v_string);
  *struct_size = sizeof(NT_Value);
  return value;
}
...
struct NT_Value* NT_GetValueRawForTesting(uint64_t last_change, const char* raw,
  ...
  value->type = NT_RAW;
  value->last_change = last_change;
  nt::ConvertToC(wpi_llvm::StringRef(raw, raw_len), &value->data.v_string);
  *struct_size = sizeof(NT_Value);
  return value;
}
...
static void CopyNtString(const struct NT_String* copy_from,
  ...
  nt::ConvertToC(wpi_llvm::StringRef(copy_from->str, copy_from->len), copy_to);
}
...
struct NT_RpcParamDef* NT_GetRpcParamDefForTesting(const char* name,
  ...
  nt::ConvertToC(wpi_llvm::StringRef(name), &def->name);
...
struct NT_RpcAnswer* NT_GetRpcAnswerForTesting(
  ...
  info->entry = rpc_id;
  info->call = call_uid;
  nt::ConvertToC(wpi_llvm::StringRef(name), &info->name);
  nt::ConvertToC(wpi_llvm::StringRef(params, params_len), &info->params);
