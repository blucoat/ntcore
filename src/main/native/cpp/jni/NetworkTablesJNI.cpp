...
std::shared_ptr<nt::Value> FromJavaBooleanArray(JNIEnv* env, jbooleanArray jarr,
  ...
  if (!ref) return nullptr;
  wpi_llvm::ArrayRef<jboolean> elements{ref};
...
static jobjectArray MakeJObject(JNIEnv *env, jobject inst,
                                wpi_llvm::ArrayRef<nt::ConnectionNotification> arr) {
...
static jobjectArray MakeJObject(JNIEnv *env, jobject inst,
                                wpi_llvm::ArrayRef<nt::EntryNotification> arr) {
...
static jobjectArray MakeJObject(JNIEnv *env, jobject inst,
                                wpi_llvm::ArrayRef<nt::LogMessage> arr) {
...
static jobjectArray MakeJObject(JNIEnv *env, jobject inst,
                                wpi_llvm::ArrayRef<nt::RpcAnswer> arr) {
  ...
  (JNIEnv *env, jclass, jint inst, jstring filename)
{
  ...
  const char* err = nt::LoadPersistent(inst, JStringRef{env, filename}.str(),
                                       [&](size_t line, const char* msg) {
                                         wpi_llvm::SmallString<128> warn;
                                         wpi_llvm::raw_svector_ostream oss(warn);
                                         oss << line << ": " << msg;
