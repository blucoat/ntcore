...
class LoadPersistentImpl {
 ...
 private:
  ...
  bool ReadHeader();
  NT_Type ReadType();
  wpi_llvm::StringRef ReadName(wpi_llvm::SmallVectorImpl<char>& buf);
  ...
  wpi_llvm::StringRef m_line;
  wpi_llvm::SmallString<128> m_line_buf;
  size_t m_line_num = 0;
 ...
 * tail string.
 */
static std::pair<wpi_llvm::StringRef, wpi_llvm::StringRef> ReadStringToken(
    wpi_llvm::StringRef source) {
  // Match opening quote
  if (source.empty() || source.front() != '"')
    return std::make_pair(wpi_llvm::StringRef(), source);
...
static wpi_llvm::StringRef UnescapeString(wpi_llvm::StringRef source,
...
bool LoadPersistentImpl::Load(StringRef prefix, std::vector<Entry>* entries) {
  ...
  while (ReadLine()) {
    ...
    wpi_llvm::StringRef name = ReadName(buf);
...
wpi_llvm::StringRef LoadPersistentImpl::ReadName(wpi_llvm::SmallVectorImpl<char>& buf) {
  wpi_llvm::StringRef tok;
...
bool Storage::LoadEntries(
  ...
  wpi_llvm::SmallString<128> prefixBuf;
