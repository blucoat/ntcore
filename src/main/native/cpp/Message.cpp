...
std::shared_ptr<Message> Message::Read(WireDecoder& decoder,
  ...
  switch (msg_type) {
    ...
    case kExecuteRpc: {
      ...
      msg->m_str = wpi_llvm::StringRef(params, size);
      break;
    }
    case kRpcResponse: {
      if (decoder.proto_rev() < 0x0300u) {
      ...
      msg->m_str = wpi_llvm::StringRef(results, size);
      break;
    }
    default:
...
std::shared_ptr<Message> Message::ClientHello(wpi_llvm::StringRef self_id) {
...
std::shared_ptr<Message> Message::EntryAssign(wpi_llvm::StringRef name,
                                              unsigned int id,
