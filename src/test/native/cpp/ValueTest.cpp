...
TEST_F(ValueTest, String) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("hello"), cv.data.v_string.str);
  ...
  ConvertToC(*v, &cv);
  ASSERT_EQ(NT_STRING, cv.type);
  ASSERT_EQ(wpi_llvm::StringRef("goodbye"), cv.data.v_string.str);
...
TEST_F(ValueTest, BooleanArray) {
  ...
  ASSERT_EQ(wpi_llvm::ArrayRef<int>(vec), v->GetBooleanArray());
  NT_Value cv;
  NT_InitValue(&cv);
  ConvertToC(*v, &cv);
...
TEST_F(ValueTest, DoubleArray) {
  ...
  ASSERT_EQ(wpi_llvm::ArrayRef<double>(vec), v->GetDoubleArray());
  NT_Value cv;
  NT_InitValue(&cv);
  ConvertToC(*v, &cv);
...
TEST_F(ValueTest, StringArray) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("hello"), v->GetStringArray()[0]);
  ASSERT_EQ(wpi_llvm::StringRef("goodbye"), v->GetStringArray()[1]);
  ASSERT_EQ(wpi_llvm::StringRef("string"), v->GetStringArray()[2]);
  NT_Value cv;
  NT_InitValue(&cv);
  ConvertToC(*v, &cv);
  ASSERT_EQ(NT_STRING_ARRAY, cv.type);
  ASSERT_EQ(3u, cv.data.arr_string.size);
  ASSERT_EQ(wpi_llvm::StringRef("hello"), cv.data.arr_string.arr[0].str);
  ASSERT_EQ(wpi_llvm::StringRef("goodbye"), cv.data.arr_string.arr[1].str);
  ASSERT_EQ(wpi_llvm::StringRef("string"), cv.data.arr_string.arr[2].str);
  ...
  ASSERT_EQ(NT_STRING_ARRAY, v->type());
  ASSERT_EQ(3u, v->GetStringArray().size());
  ASSERT_EQ(wpi_llvm::StringRef("s1"), v->GetStringArray()[0]);
  ASSERT_EQ(wpi_llvm::StringRef("str2"), v->GetStringArray()[1]);
  ASSERT_EQ(wpi_llvm::StringRef("string3"), v->GetStringArray()[2]);
  ConvertToC(*v, &cv);
  ASSERT_EQ(NT_STRING_ARRAY, cv.type);
  ASSERT_EQ(3u, cv.data.arr_string.size);
  ASSERT_EQ(wpi_llvm::StringRef("s1"), cv.data.arr_string.arr[0].str);
  ASSERT_EQ(wpi_llvm::StringRef("str2"), cv.data.arr_string.arr[1].str);
  ASSERT_EQ(wpi_llvm::StringRef("string3"), cv.data.arr_string.arr[2].str);
  ...
  ASSERT_EQ(wpi_llvm::StringRef("short"), v->GetStringArray()[0]);
  ASSERT_EQ(wpi_llvm::StringRef("er"), v->GetStringArray()[1]);
  ConvertToC(*v, &cv);
  ASSERT_EQ(NT_STRING_ARRAY, cv.type);
  ASSERT_EQ(2u, cv.data.arr_string.size);
  ASSERT_EQ(wpi_llvm::StringRef("short"), cv.data.arr_string.arr[0].str);
  ASSERT_EQ(wpi_llvm::StringRef("er"), cv.data.arr_string.arr[1].str);
