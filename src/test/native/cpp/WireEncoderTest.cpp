...
class WireEncoderTest : public ::testing::Test {
 protected:
  WireEncoderTest() {
    ...
    v_string = Value::MakeString(wpi_llvm::StringRef("hello"));
    v_raw = Value::MakeRaw(wpi_llvm::StringRef("hello"));
...
TEST_F(WireEncoderTest, Write8) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x05\x01\x00", 3),
            wpi_llvm::StringRef(e.data(), e.size()).substr(off));
}
...
TEST_F(WireEncoderTest, Write16) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x05\x00\x01\x45\x67\x00\x00", 8),
            wpi_llvm::StringRef(e.data(), e.size()).substr(off));
}
...
TEST_F(WireEncoderTest, Write32) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x00\x00\x05\x00\x00\x00\x01\x00\x00\xab\xcd"
...
TEST_F(WireEncoderTest, WriteDouble) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x00\x00\x00\x00\x00\x00\x00"
...
TEST_F(WireEncoderTest, WriteUleb128) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x7f\x80\x01", 4),
            wpi_llvm::StringRef(e.data(), e.size()).substr(off));
}
...
TEST_F(WireEncoderTest, WriteType) {
  ...
  ASSERT_EQ(8u, e.size() - off);
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x01\x02\x03\x10\x11\x12\x20", 8),
            wpi_llvm::StringRef(e.data(), e.size()).substr(off));
}
...
TEST_F(WireEncoderTest, WriteBooleanValue2) {
  ...
  ASSERT_EQ(nullptr, e.error());
  ASSERT_EQ(2u, e.size());
  ASSERT_EQ(wpi_llvm::StringRef("\x01\x00", 2),
            wpi_llvm::StringRef(e.data(), e.size()));
}
...
TEST_F(WireEncoderTest, WriteDoubleValue2) {
  ...
  ASSERT_EQ(nullptr, e.error());
  ASSERT_EQ(8u, e.size());
  ASSERT_EQ(wpi_llvm::StringRef("\x3f\xf0\x00\x00\x00\x00\x00\x00", 8),
            wpi_llvm::StringRef(e.data(), e.size()));
}
...
TEST_F(WireEncoderTest, WriteStringValue2) {
  ...
  ASSERT_EQ(nullptr, e.error());
  ASSERT_EQ(7u, e.size());
  ASSERT_EQ(wpi_llvm::StringRef("\x00\x05hello", 7),
            wpi_llvm::StringRef(e.data(), e.size()));
}
...
TEST_F(WireEncoderTest, WriteBooleanArrayValue2) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x03\x00\x01\x00", 4),
            wpi_llvm::StringRef(e.data(), e.size()));
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\xff\x00", 2), wpi_llvm::StringRef(e.data(), 2));
}
...
TEST_F(WireEncoderTest, WriteDoubleArrayValue2) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x02\x3f\xe0\x00\x00\x00\x00\x00\x00"
...
TEST_F(WireEncoderTest, WriteStringArrayValue2) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x02\x00\x05hello\x00\x07goodbye", 17),
            wpi_llvm::StringRef(e.data(), e.size()));
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\xff\x00\x01", 3), wpi_llvm::StringRef(e.data(), 3));
}
...
TEST_F(WireEncoderTest, WriteStringValue3) {
  ...
  ASSERT_EQ(nullptr, e.error());
  ASSERT_EQ(6u, e.size());
  ASSERT_EQ(wpi_llvm::StringRef("\x05hello", 6),
            wpi_llvm::StringRef(e.data(), e.size()));
}
...
TEST_F(WireEncoderTest, WriteStringArrayValue3) {
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\x02\x05hello\x07goodbye", 15),
            wpi_llvm::StringRef(e.data(), e.size()));
  ...
  ASSERT_EQ(wpi_llvm::StringRef("\xff\x01", 2), wpi_llvm::StringRef(e.data(), 2));
}
...
TEST_F(WireEncoderTest, WriteString2) {
  ...
  EXPECT_EQ(nullptr, e.error());
  EXPECT_EQ(7u, e.size());
  EXPECT_EQ(wpi_llvm::StringRef("\x00\x05hello", 7),
            wpi_llvm::StringRef(e.data(), e.size()));
  ...
  EXPECT_EQ(wpi_llvm::StringRef("\x00\x80**", 4), wpi_llvm::StringRef(e.data(), 4));
  ...
  EXPECT_EQ(wpi_llvm::StringRef("\xff\xff**", 4), wpi_llvm::StringRef(e.data(), 4));
...
TEST_F(WireEncoderTest, WriteString3) {
  ...
  EXPECT_EQ(nullptr, e.error());
  EXPECT_EQ(6u, e.size());
  EXPECT_EQ(wpi_llvm::StringRef("\x05hello", 6),
            wpi_llvm::StringRef(e.data(), e.size()));
  ...
  EXPECT_EQ(nullptr, e.error());
  ASSERT_EQ(130u, e.size());
  EXPECT_EQ(wpi_llvm::StringRef("\x80\x01**", 4), wpi_llvm::StringRef(e.data(), 4));
  EXPECT_EQ('*', e.data()[128]);
  EXPECT_EQ('x', e.data()[129]);
  ...
  EXPECT_EQ(wpi_llvm::StringRef("\x81\x80\x04*", 4), wpi_llvm::StringRef(e.data(), 4));
